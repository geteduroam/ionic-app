...
public class WifiEapConfigurator: CAPPlugin {
    ...
    @objc func configureAP(_ call: CAPPluginCall) {
        ...
        let ssids = call.getArray("ssid", [String: Any].self)
        let oids = getArray("oid", [String: Any].self)

        if (oids.count == 0  && ssids.count == 0) {
            return call.success([
    ...
    func configureNetwork (config: NEHotspotConfiguration, _ call: CAPPluginCall) {
