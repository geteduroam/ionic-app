...
export class GeteduroamServices {
  ...
  async connectProfile(config) {
    let resultantProfiles = this.global.getCredentialApplicability()
      ...
      : []
      ;
    console.log('resultantProfiles: ', resultantProfiles);
    config['id'] = this.id;
    ...
    console.log('connectProfile config: ', config);
   ...
   */
  async generateOAuthFlow(data: any) {
    let codeVerifier = GeteduroamServices.generateRandomString(43);
    let codeChallenge = await GeteduroamServices.deriveChallenge(codeVerifier);
    let state = GeteduroamServices.generateRandomString(20);
    return {
      codeVerifier,
      codeChallenge,
      state
  ...
  static deriveChallenge(codeVerifier: string): Promise<string> {
    ...
    if (!GeteduroamServices.HAS_SUBTLE_CRYPTO) {
